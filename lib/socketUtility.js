...
export class SocketClient{
    ...
    init(){
        const that = this;
        // Capture the socket this init() belongs to. `this._ws` can be swapped out
        // from under us by the `ws` setter (reconnect), and a heartbeat that reads
        // the live field would keep pinging — and eventually close — the NEW socket.
        const ws = this._ws;
        ...
        this._ws.onopen = function(){
            ...
            const interval = setInterval(()=>{
                if (that._ws !== ws){        // superseded by a reconnect; this heartbeat is done
                    clearInterval(interval);
                } else if (ws.readyState > 1 || that.pingPongDelta > that.cfg.keepAliveTimeout / that.cfg.heartBeatInterval){
                    ws.close();
